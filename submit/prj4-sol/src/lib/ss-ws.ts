...
async function doFetchJson<T>(method: string, url: URL, jsonBody?: object): Promise<Result<T>> {
  try {
    ...
    if (!response.ok) {
      // Handle HTTP errors
      return errResult(new Err(`HTTP error: ${response.statusText}`, { code: 'http_error' }));
    }

    const responseBody = await response.json();

    if (responseBody.isOk === false) {
      // Handle web service errors
      const errors = responseBody.errors.map((error: any) => new Err(error.message, error.options));
      return errResult(errors);
    }

    // If no errors, return the response result
    return okResult(responseBody as T);
  } catch (error) {
    // Convert any exceptions into an error Result
    return errResult(new Err(`Error: ${error.message}`, { code: 'fetch_error' }));
  }
}
